...
var sqlMap = {
    ...
    user: {
        ...
        addvideosql:'Insert into uservideo(user,video,videologo,createtime) value(?,?,?,?)',//保存文章视频
        getvideosql:'SELECT video,videologo FROM uservideo WHERE user = ?',  //获取用户上传的所有视频
    ...
    admin:{
        login: 'SELECT password FROM admin WHERE Account = ?;',//登录
        updatetoken:'UPDATE admin SET token=? where Account=?'
    }
}
